...
PiArray.addMethod = function (array, name, cb) {
    const cbs = [];
...
class PiVirtualTree {
    ...
    _createComponents() {
        for (let i = 0; i < this.#componentsParams.length; i++) {
            ...
            const component = this._createComponent(componentName);

            if (this.#componentParent && name != null) {
    ...
    _createVirtualElements(elements) {
        ...
        for (let i = 0; i < elements.length; i++) {
            ...
            if (element._pie == undefined) {
                ...
                const velement = new PiElement(element, this.#scope);
...
class PiTree {
    ...
    _loadDocument(html) {
        try {
            const wp = new PiEnv.global.DOMParser();
...
class PiExpression {
    ...
    constructor(template) {
        ...
        this._rxExpression = PiConfig.get('expression.xlanguage');
        this._rxVariable = PiConfig.get('expression.xvariable');
    ...
    parse(context, variables = [[], []]) {
        ...
        for (let i = 0; i < expressions.length; i++) {
            ...
            let value = PiStatement.execWithVariables(stmt, variables, context);
 ...
 * Função vazia
 */
PiFunction.noop = function () { };(function () {
    ...
    PiExport('PiForDirective', class {
        ...
        _loadNumber(n) {
            ...
            const isNumber = PiType.isNumber(number);
    ...
    const stmtForearchWithIndex = /^\((\w+)\s*,\s*(\w+)\)\s+in\s+(\w+)$/gi;

    PiExport('PiForEachDirective', class {
...
class PiElement {
    ...
    extendForeachContext(foreachContext) {
        PiObject.extend(this._foreachContext, foreachContext);
    ...
    addEventStatement(statement, event) {
        ...
        this.listen(event, function (e) {
            ...
            const variables = defaultVariables.concat([['$event'], [e]]);

            const stmt = expression.parse(ctx, variables);

            return PiStatement.execWithVariables('this.' + stmt, variables, ctx);
        });
    ...
    setForDirective(stmt) {
        ...
        forDirective
            .loop((ixn, index) => {
                ...
                const vtree = new PiVirtualTree(clonedElement.children, this._scope);
    ...
    setForEachDirective(stmt) {
        ...
        PiArray.addMethod(array, 'load', function (array) {
            this.splice(0, 0, ...array);

            return this;
        });

        PiArray.addMethod(array, 'add', function (item) {
            this.splice(this.length, 0, item);

            return this;
        });

        PiArray.addMethod(array, 'clear', function () {
            this.splice(0, this.length);

            return this;
        });

        PiArray.addMethod(array, 'remove', function (index) {
            this.splice(index, 1);

            return this;
        });

        PiArray.addMethod(array, 'reload', function () {
        ...
        PiArray.addMethod(array, 'splice', function (index, deleteCount, ...items) {
            if (deleteCount > 0) {
    ...
    _getDefaultVariables() {
        ...
        const variablesValues = [self, this._element, this._element.get()];
        const variablesForeachNames = Object.keys(this._foreachContext);
        const variablesForeachValues = Object.values(this._foreachContext);

        return [variablesNames.concat(variablesForeachNames), variablesValues.concat(variablesForeachValues)];
    }
    ...
    _loadTemplate() {
        ...
        for (let i = 0; i < nodes.length; i++) {
            ...
            if (node instanceof Text) {
                const exp = new PiExpression(node.nodeValue);
                node._piex = exp;
    ...
    _updateView() {
        ...
        for (let i = 0; i < nodes.length; i++) {
            const node = nodes[i];
            if (node._piex) {
                node.nodeValue = node._piex.parse(this._getContext(), this._getDefaultVariables());
            }
        }
    }
...
class PiComponent extends PiClass {
    ...
    inject(model) {
        if (model instanceof PiModel) {
            const names = PiString.clips(this.view, 'data-model="', '"');
    ...
    viewLoad(element, append = false) {
        if (this.view) {
            const root = PiTree.parse(this.view);
            const vtree = new PiVirtualTree(root, this.#scope, this);
            vtree.load();
     ...
     */
    watch(property, fn) {
        const ids = PiObject.on(this.#scope, 'context.' + property, fn)
    ...
    unWatch(property) {
        ...
        for (let i = 0; i < arr.length; i++) {
            if (arr[i].property == property) {
                PiObject.off(this.#scope, arr[i].ids);
                break;
            }
        }
    ...
    #listen() {
        this.element.addEventListener('DOMNodeInserted', (e) => {
            const vtree = new PiVirtualTree(e.target, this.#scope, this);
            vtree.load();
            this.#vtrees.push(vtree);
        });
    }
    ...
    #loadAttributes(element) {
        ...
        for (let i = 0; i < attrs.length; i++) {
            let value = element.getAttribute(attrs[i]);
...
class PiApp extends PiClass {
     ...
     */
    servicesStart() {
        const services = this._servicesCreated;
        let totalRouteNotFound = 0;
    ...
    #createServices() {
        const services = this._servicesLoaded;
        for (let i = 0; i < services.length; i++) {
            ...
            try {
                ...
                this._servicesCreated.push(service);
            } catch (error) {
...
class PiBoot extends PiClass {
    ...
    #createVirtualTree() {
        ...
        const root = PiTree.parse(element.innerHTML);
        const vtree = new PiVirtualTree(root, app._scope, app);
        vtree.load();
